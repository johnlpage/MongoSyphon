...
It has been designed specifically to target the  MongoDB and to make use of the powerful update facilities it provides. This differs from many other ETL tools which primarily target relational targets potentially with a minimal MongoDB add-on. MongoSyphon can be used both for an initial bulk transfer and for ongoing updates.
